...
nvinfer1::IPluginV2* PluginSlice( int x1, int y1, int z1, int x2, int y2, int z2)
{
    //Use the extern function getPluginRegistry to access the global TensorRT Plugin Registry
    auto creator = getPluginRegistry()->getPluginCreator("LICE_TRT", "1");
    // const PluginFieldCollection* pluginFC = creator->getFieldNames();
    //populate the field parameters (say layerFields) for the plugin layer 
    nvinfer1::PluginField xx[7];
    xx[0]=nvinfer1::PluginField("x1",&x1,PluginFieldType::kINT32);
    xx[1]=nvinfer1::PluginField("y1",&y1,PluginFieldType::kINT32);
    xx[2]=nvinfer1::PluginField("z1",&z1,PluginFieldType::kINT32);
    xx[3]=nvinfer1::PluginField("x2",&x2,PluginFieldType::kINT32);
    xx[4]=nvinfer1::PluginField("y2",&y2,PluginFieldType::kINT32);
    xx[5]=nvinfer1::PluginField("z2",&z2,PluginFieldType::kINT32);
    xx[6]=nvinfer1::PluginField("type",&z2,PluginFieldType::kFLOAT32);

    // nvinfer1::PluginFieldCollection layerFields=PluginFieldCollection(xx);
    ...
    IPluginV2* pluginObj = creator->createPlugin("LICE_TRT", &pluginData);
    assert(pluginObj);
    ...
    SampleUniquePtr<nvinfer1::INetworkDefinition>& network, SampleUniquePtr<nvinfer1::IBuilderConfig>& config)
{
    ...
    if(mParams.fp16)
    {
        input_type=DataType::kHALF;
    }
    else
    {
        input_type=DataType::kFLOAT;
    }

    ITensor* input1 = network->addInput(
        "input1", input_type, Dims3{1,32,4});
    ITensor* input2 = network->addInput(
        "input2", input_type, Dims3{2,1,4});
    ...
    //  auto slice=network->addElementWise(*input1,*input2,nvinfer1::ElementWiseOperation::kSUM);

    // auto slice=network->addElementWise(*input1,*input2,nvinfer1::ElementWiseOperation::kSUM);
    auto slice=network->addPluginV2(&inputs[0],2,*PluginSlice(mParams.x1,mParams.y1,mParams.z1,mParams.x2,mParams.y2,mParams.z2));
    for(int i=0;i<2;i++)
    {
        std::vector<ITensor*> inputs_Concat; 
        inputs_Concat.push_back(slice->getOutput(0));
        inputs_Concat.push_back(input2);
        auto sum=network->addConcatenation(&inputs_Concat[0],2);
        sum->setAxis(1);
        auto shape=sum->getOutput(0)->getDimensions();
        std::cout<<"concat shape ";
        ...
        inputs_.push_back(sum->getOutput(0));
        slice=network->addPluginV2(&inputs_[0],2,*PluginSlice(mParams.x1,mParams.y1,mParams.z1,mParams.x2,mParams.y2,mParams.z2));
    }
    slice->getOutput(0)->setName(mParams.outputTensorNames[0].c_str());
    auto shape=slice->getOutput(0)->getDimensions();
    ...
    // profile->setDimensions("input", OptProfileSelector::kMIN, Dims{ 1, 1 ,4});
    // profile->setDimensions("input", OptProfileSelector::kOPT, Dims{ 1, 1,4 });
    // profile->setDimensions("input", OptProfileSelector::kMAX, Dims{ 1, 1,4 });
        // Build engine
...
bool SampleTest::processInput(const samplesCommon::BufferManager& buffers)
{
    ...
    if(mParams.fp16)
    {
        half* hostDataBuffer = static_cast<half*>(buffers.getHostBuffer(mParams.inputTensorNames[0]));
        for (int i = 0; i < mParams.x1 * mParams.y1*mParams.z1; i++)
        {
            hostDataBuffer[i] = __float2half(100.0);
        }
        hostDataBuffer = static_cast<half*>(buffers.getHostBuffer(mParams.inputTensorNames[1]));
...
bool SampleTest::verifyOutput(const samplesCommon::BufferManager& buffers)
{
    ...
    if(mParams.fp16)
    {
        half* prob = static_cast<half*>(buffers.getHostBuffer(mParams.outputTensorNames[0]));
        std::cout << "\nOutput:\n" << std::endl;
        for (int i = 0; i < mParams.x2 * mParams.y2*mParams.z2; i++)
        {
            
            std::cout << i << ": " << __half2float(prob[i])  << std::endl;
        }
        std::cout << std::endl;
    }
    else
    {
        ...
        std::cout << "\nOutput:\n" << std::endl;
        for (int i = 0; i < mParams.x2 * (mParams.y2+3)*mParams.z2*2; i++)
        {
